...
///
/// It's up to the client application to provide an implementation of this trait. During rendering, the `evaluate()` method will be called for each sample
/// generated by the sampler.
///
/// Note that samples are in the range (0..1, 0..1). Render function implementations do not need to be aware of the size and resolution of the output raster
/// (perhaps only on the aspect ratio of the raster).
pub trait RenderFunction<V: Copy + Default> {
    /// Evaluates the render function at sample point (x, y), which are in the range (0..1, 0..1).
    fn evaluate(&self, x: f32, y: f32) -> V;
}

/// Renders a raster using the specified sampler, render function and filter.
